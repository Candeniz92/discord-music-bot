...
module.exports = {
   ...
   if(!player) return message.channel.send({ embeds: [new MessageEmbed().setDescription(`Nothing is playing right now.`)]});
            
            if(!player.queue) return message.channel.send({ embeds: [new MessageEmbed().setDescription(`Nothing is playing right now.`)]});

            if(player.queue.length === "0" || !player.queue.length) {
                const embed = new MessageEmbed()
                .setDescription(`No Queue`)
