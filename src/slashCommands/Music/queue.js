...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        if (!player.queue.size || player.queue.size === 0) {
            ...
            const embed = new MessageEmbed().setDescription(`No Queue`);
            ...
            if (player.queue.size < 10 || player.queue.totalSize < 10) {
                ...
                const embed2 = new MessageEmbed().setAuthor({ name: `${interaction.guild.name} Server Queue`}).setDescription(`**Now playing**\n[${player.queue.current.title}](${player.queue.current.uri}) [${player.queue.current.requester}]\n\n**Queued Songs**\n${pages[page]}`).setFooter({ text: `Page ${page + 1}/${pages.length}`})

                await interaction.editReply({
                    embeds: [embed2]
                }).catch(() => { });
            } else {
                const embed3 = new MessageEmbed().setAuthor({ name: `${interaction.guild.name} Server Queue`}).setDescription(`**Now playing**\n[${player.queue.current.title}](${player.queue.current.uri}) [${player.queue.current.requester}]\n\n**Queued Songs**\n${pages[page]}`).setFooter({ text: `Page ${page + 1}/${pages.length}`})
                ...
                collector.on("collect", async (button) => {
                    if (button.customId === "queue_cmd_but_1_app") {
                        ...
                        const embed4 = new MessageEmbed().setDescription(`**Now playing**\n[${player.queue.current.title}](${player.queue.current.uri}) [${player.queue.current.requester}]\n\n**Queued Songs**\n${pages[page]}`).setFooter({ text: `Page ${page + 1}/${pages.length}`})
                        ...
                        const embed5 = new MessageEmbed().setDescription(`**Now playing**\n[${player.queue.current.title}](${player.queue.current.uri}) [${player.queue.current.requester}]\n\n**Queued Songs**\n${pages[page]}`).setFooter({ text: `Page ${page + 1}/${pages.length}`})
